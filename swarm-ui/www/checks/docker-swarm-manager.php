...
$checkSwarm = array();
$arrlength = count($swarmInfos['responce']);

for($x = 0; $x < $arrlength; $x++){
  $findme = "/swarm manage";
  $checkCommande = strpos($swarmInfos['responce'][$x]['Command'], $findme);
  if ( $checkCommande === 0 ){
    list($empty, $nodeName, $serviceName) =  explode("/", $swarmInfos['responce'][$x]['Names'][0]);
    list($status) =  explode(" ", $swarmInfos['responce'][$x]['Status']);
