<?php
	...
	$req="update ds_tasks set status_id = '8', start_date = '".$date_jobs."', progress = '100' where job_id = '".$job_id."' and action_id = '".$actions_name['Start agent']."';";
	$resu = execRequete ($req, $connexion);
	
	// Execution Pull Docker Image
	$fileout=$server['cfg']['logs'].$hostname.".".$job_id.".log";
	$fileurl="/logs/".$hostname.".".$job_id.".log";
	$role="docker-start-agent";
	$ansible_args = "";
	$ansiblePlay = ansiblePlay($server,$hostname,$fileout,$role,$ansible_args);
	if ( $ansiblePlay == 0 ) {
		$ansible_status=$status_name['success'];
		$req="update ds_hosts_agents set status_id = ".$status_name['running'].", random = '".rand()."' where host_id = '".$host_id."'";
		$resu = execRequete ($req, $connexion);
	} else {
	...
	$req="update ds_tasks set status_id = '".$ansible_status."', end_date = '".$date_jobs."', log_file ='".$fileurl."', "
		. "progress = '100' where job_id = '".$job_id."' and action_id = '".$actions_name['Start agent']."'";
	$resu = execRequete ($req, $connexion);
